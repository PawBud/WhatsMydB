...
unsigned int getBufferNumberOfFrames(AudioUnit const audioUnit)
{
    ...
    OSStatus status = AudioUnitGetProperty(audioUnit, kAudioDevicePropertyBufferFrameSize, kAudioUnitScope_Global, 0,
                                           &bufferNumberOfFrames, &propertySize);
    if (status != noErr)
    {
                          ...
                          AudioBufferList* ioData)
{
    ...
    if (streamFormat.mFormatFlags & kAudioFormatFlagIsNonInterleaved)
    {
        ...
        unsigned int propsize = offsetof(AudioBufferList, mBuffers) + (sizeof(AudioBuffer) * streamFormat.
            mChannelsPerFrame);
...
void SetupAudioTap(AudioDeviceID deviceID)
{
    ...
    OSStatus status = AudioObjectGetPropertyData(deviceID, &propertyAddress, 0, nullptr, &stream_format_size,
                                                 &stream_format);
    if (status != noErr)
    {
    ...
    std::cout << "Format ID: '" << formatID << "' (0x" << std::hex << stream_format.mFormatID << std::dec << ")" <<
        std::endl;
